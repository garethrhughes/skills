...
## Глубина архитектурных артефактов (обязательно)

Архитектурный документ «готов» только когда проработан на **всех уровнях C4**: C1 context → C2 container → C3 component → C4 code. Остановка на высокоуровневом наброске C2/C3 = «набросок», не артефакт — ревью не пройдено.

Обязательные артефакты для эпика архитектуры (если не указано иное в задаче):

- context, container, component, code;
- **deployment-диаграмма** (топология, сети, зоны);
- **sequence-диаграммы** ключевых сценариев (входящий запрос, эскалация/HITL, обновление БД/БЗ, отказ очереди, пик нагрузки);
- **ER-модель** (сущности, ключевые колонки, индексы, RLS);
- **API-спецификация** (OpenAPI для API + контракты webhook/конвертов).

Каждый документ уровня содержит: элементы, отношения, решения (с альтернативами), допущения, ответы на каталог «а что если». Критерий глубины: по каждому решению можно ответить «как это реализуется, как тестируется, как откатывается».

## Каталог «А что если» (обязательный для архитектора)

Архитектор обязан ответить на **каждый** пункт каталога (механизм + ссылка на компонент) в разделе «А что если» документа. Пропуск пункта или ответ «не рассматривали» = ревью не пройдено. Не применимо → явно пометить out-of-scope с причиной:
...
- промпты: где хранятся, версионируются, тестируются (репозиторий промптов, prompt_version, ревью-гейт, ролбэк);
...
## Челлендж-гейт архитектуры (обязателен перед in_review)

- Перед переводом архитектурной задачи в `in_review` агент обязан прогнать каталог «а что если» (раздел выше) + категории: промпты/версионирование, шифрование at-rest/in-transit, DDoS L3/L4/L7, zero-downtime обновления, отказы компонентов по причинам, деградация (что никогда не деградирует), безопасность, стоимость, лицензии, расширяемость.
- Результат челленджа включается в документ (раздел «А что если» / «Челлендж»); вопросы без ответа — блокер in_review.
